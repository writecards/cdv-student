...
function gotWildfireData(incomingData){
    ...
    let newData = transformData(incomingData);
                    function transformData(dataToTransform){
                        let newData1 = dataToTransform.filter(heatMapfilterFunction);
                        return newData1;
                    }
   ...
   // console.log(newData)
                    ...
                    let maxAcres = d3.max(newData, d => d.incident_acres_burned)
                    console.log(maxAcres)

           
            minYear = d3.min(newData, getFireYear)
            
            maxYear = d3.max(newData, getFireYear) 
            let numYears = maxYear-minYear;
           
            
             xScaleHeatmap = d3.scaleLinear().domain([minYear, maxYear+1]).range([heatmapPadding, heatmapWidth-(heatmapPadding)]);
           ...
           // let xAxis = d3.axisBottom(xScaleHeatmap)
            ...
            let xAxisPos = heatmapHeight - heatmapPadding;
            ...
            yAxisGroup.call(yAxis).attr("transform", "translate(" + heatmapPadding + ", 0)");
 
    

    
  
        let acresDomain = d3.extent(newData, d => d.incident_acres_burned)
       
        console.log(acresDomain)
            
            dataGroup = heatmapviz.selectAll(".dataGroup").data(newData).enter()
                        .append("g")
             ...
             dataGroup.append("rect")
                      ...
                      .attr("fire-month", getFireMonth)
                        ...
                        .attr("width", function(d,i){
                            ...
                            let w =  (heatmapWidth - (2*heatmapPadding)) / (numYears*12)
                            return w;
...
function heatMapfilterFunction(d){
